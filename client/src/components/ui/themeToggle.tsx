...
export const ThemeToggle = ({ className }: { className?: string }) => {
    ...
    return (
        <div className={cn("flex h-8 justify-center items-center p-6", className)}>
            {themes.map(({ key, icon: Icon, label }) => {
                const isActive = mounted ? theme === key : false;
                return (
                    <Tooltip key={key}>
                        <TooltipTrigger asChild>
                            <button aria-label={label} className="relative h-6 w-6 rounded-full" onClick={() => handleThemeClick(key as "light" | "dark" | "system")} type="button">
                                {isActive && (
